...
const EditRecipe = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setCurrentImage(data.recipe.image_path ?? null)
      setPreview(data.recipe.image_path ?? null)
      setLoading(false)
    }
    load()
  }, [id, navigate])
  ...
  return (
    <div className="page-container space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-ink dark:text-white">Edit Recipe</h1>
          <p className="text-sm text-muted">Update your recipe details</p>
        </div>
      </div>
        ...
        <div>
          ...
          {preview ? (
            ...
            <label className="flex flex-col items-center justify-center h-48 border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl cursor-pointer hover:border-accent/50 transition-colors">
              ...
              <span className="text-sm text-muted">Click to upload a new image</span>
              <input
                type="file"
                accept="image/*"
